    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if existing_task:
                ...
                # If the task does not exist, you can choose to insert it, if needed
                # For example:
                await db.execute(
                    text("""
        ...
        raise HTTPException(status_code=500, detail=f"Error cleaning and updating data: {str(e)}")
...
async def analyze_overdue(db: AsyncSession = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing overdue tasks: {str(e)}")
...
async def analyze_completion_time(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing completion time: {str(e)}")
...
async def download_tasks_csv(db: AsyncSession = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error downloading tasks CSV: {str(e)}")
